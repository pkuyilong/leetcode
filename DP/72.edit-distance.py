...
dp[i][j]表示当word1在i位置，word2在j位置时，最小的改动次数， 要将word1变成word2
word1  1....i
word2  1....j
dp[i][j]表示word1的最后一个下标为i的子序列和word2以j为下标的子序列的相同的最少编辑次数
1. 如果字符不同：
    1.1 可以在第一个单词后边插入一个字符
        a b c e                  a b c d e
        a b c d     修改完之后     a b c d
        这种情况下，需要将word1 的 c 字母后边添加d 才能保证一样，
        也就是说是在 word1[1....i] 与 word2[1....j-1]是相同的
    1.2 可以在第一个单词中删除一个字符
        dp[i-1][j]
